...
echo ""
echo "<details>"
echo "<summary><h3><a href=\"$PROBLEM_URL\">$PROBLEM_NUMBER. $PROBLEM_TITLE</a>$POTD</h3></summary>"
echo ""
echo "\`$DIFFICULTY\` \`Time Beats: $TIME_BEATS\` \`Memory Beats: $MEMORY_BEATS\` \`Commit:$COMMIT_HASH\` \`Solved At: $COMMIT_TIME\`"
echo ""
echo '```'"$EXT"
echo "$CODE_CONTENT"
echo '```'
